...
if [[ "$count" == "1" ]]; then
  printf '{"status":"ok","queue":{"busy":false,"runs":1,"events":2,"unfinished_runs":1}}\\n'
else
...
def test_idle_restart_helper_times_out_during_dequeued_unfinished_run(tmp_path):
    calls = tmp_path / "calls"
    fake_launchctl = tmp_path / "launchctl"
    fake_curl = tmp_path / "curl"
    write_executable(
...
""",
    ...
    write_executable(
        fake_curl,
        """#!/bin/bash
printf '{"status":"ok","queue":{"busy":false,"runs":0,"events":0,"unfinished_runs":1}}\\n'
""",
    )
    env = os.environ | {
        "LAUNCHCTL_BIN": str(fake_launchctl),
...
def test_idle_restart_helper_fails_closed_without_unfinished_run_signal(tmp_path):
    calls = tmp_path / "calls"
    fake_launchctl = tmp_path / "launchctl"
    fake_curl = tmp_path / "curl"
    write_executable(
